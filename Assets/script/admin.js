...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelector("#staticBackdrop2 .btn-guardar").addEventListener("click", () => {
        ...
        if (!nombre || !cantidad || !stock) return;
    ...
    // CLIENTES
    const tablaClientes = document.getElementById("tabla-clientes1").querySelector("tbody");
    ...
    document.querySelector("#staticBackdrop .btn-guardar").addEventListener("click", () => {
        ...
        const telefono = document.getElementById("numeroTelefonoCliente").value.trim();
        const direccion = document.getElementById("direccionCliente").value.trim();
        const compras = document.getElementById("numeroComprasR").value.trim();

        if (!nombre || !correo || !telefono || !direccion || !compras) return;

        if (clienteEnEdicion) {
            clienteEnEdicion.cells[0].textContent = nombre;
            clienteEnEdicion.cells[1].textContent = correo;
            clienteEnEdicion.cells[2].textContent = telefono;
            clienteEnEdicion.cells[3].textContent = direccion;
            clienteEnEdicion.cells[4].textContent = compras;
        } else {
                ...
                <div class="button-borrar">
                    <button class="bin-button">
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 33 39" class="bin-bottom">
                            <mask fill="white" id="paths1-1-inside-1_8_19">
                                <path d="M0 0H33V35C33 37.2091 31.2091 39 29 39H4C1.79086 39 0 37.2091 0 35V0Z"></path>
                            </mask>
                            <path mask="url(#paths1-1-inside-1_8_19)" fill="white" d="M0 0H33H0ZM37 35C37 39.4183 33.4183 43 29 43H4C-0.418278 43 -4 39.4183 -4 35H4H29H37ZM4 43C-0.418278 43 -4 39.4183 -4 35V0H4V35V43ZM37 0V35C37 39.4183 33.4183 43 29 43V35V0H37Z"></path>
    ...
    function activarBotones() {
        ...
        document.querySelectorAll("#clientes .Btn-edit").forEach(btn => {
            btn.onclick = function () {
                ...
                document.getElementById("numeroTelefonoCliente").value = fila.cells[2].textContent;
                document.getElementById("direccionCliente").value = fila.cells[3].textContent;
                document.getElementById("numeroComprasR").value = fila.cells[4].textContent;
                new bootstrap.Modal(document.getElementById("staticBackdrop")).show();
            };
        });
    ...
    function limpiarModalCliente() {
        ...
        document.getElementById("numeroTelefonoCliente").value = "";
